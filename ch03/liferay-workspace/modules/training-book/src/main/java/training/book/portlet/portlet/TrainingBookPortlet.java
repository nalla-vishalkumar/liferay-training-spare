...
public class TrainingBookPortlet extends MVCPortlet {
    ...
    public void render(RenderRequest renderRequest,
        ...
        try {
            ServiceContext serviceContext = ServiceContextFactory
                    .getInstance(Training.class.getName(), renderRequest);

            long groupId = serviceContext.getScopeGroupId();

            long trainingId = ParamUtil.getLong(renderRequest, "trainingId");

            List<Training> trainings = _trainingLocalService
                    .getTrainings(groupId);

            if (trainings.isEmpty()) {
                Training training = _trainingLocalService.addTraining(
                        serviceContext.getUserId(), "Main Training",
                        "Main Training Desc", serviceContext);

                trainingId = training.getTrainingId();
            }

            if (trainingId == 0) {
                trainingId = trainings.get(0).getTrainingId();
            }

            renderRequest.setAttribute("trainingId", trainingId);
        } catch (Exception e) {
        ...
        if (tutorId > 0) {
            try {
                ...
                response.setRenderParameter("mvcPath",
                        "/training-book/edit_training.jsp");
            }
        } else {
            try {
                _tutorLocalService.addTutor(serviceContext.getUserId(), tutorId,
    ...
    public void deleteTutor(ActionRequest request, ActionResponse response)
            throws PortalException {
        long tutorId = ParamUtil.getLong(request, "tutorId");
        long trainingId = ParamUtil.getLong(request, "trainingId");
